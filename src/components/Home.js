...
const Home = () => {
    ...
    const buyMarketItem = async (item) => {
        console.log("called for itemmmmmmmmmmmmmm baby",item)
        ...
        try {
            ...
            document.getElementById(`itm${item.item.itemId}`).innerHTML = "request sent";
            document.getElementById(`itm${item.item.itemId}`).disabled = false;
    ...
    return (
        <>
            ...
            {items.allMarketItems.length > 0 ?
                <div className="px-5 container">
                <Row xs={1} md={2} lg={4} className="g-4 py-5">
                    {items.allMarketItems.map((item, idx) => (
